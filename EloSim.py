...
'''
def play_tournament(players, team_size = 1, uncertainty=32, learning_rate = 1.0, rounds=20, match_rnd=1.0, usebrackets=False, show_elo_every=None, show_dist_every=None, show_loss_every=None):
    ...
    for j in range(rounds):
        ...
        while 2*team_size*matches_played < len(players) - 1:
            ...
            for i in range((team_size*2) - 1):
                index = 0
                if int(len(playing_ppl)*match_rnd) > 0:
                    index = np.random.randint(low=0, high=int(len(playing_ppl)*match_rnd))
